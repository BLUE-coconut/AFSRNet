...
# 图像 multilevel：Raw(RSU*4) + Half-maxpool(尺寸为1/2,RSU*5)
...
class con_up_fusion(nn.Module):
    def __init__(self, in_ch=3, out_ch=3, dirate=1):
        ...
        self.confuse = Conv1x1(in_ch, in_ch)
...
class woAFF(nn.Module):
    # SFR+1/2image
    def __init__(self, in_ch=3, out_ch=1):
        super(woAFF, self).__init__()
         # level1: raw image
        ...
        # v1.0
        self.adfuse1 = Conv1x1(64, out_ch=64)  # 64,64
        self.adfuse2 = Conv1x1(32+128+64, out_ch=128)  # 160,128
        self.adfuse3 = Conv1x1(32+32+256+32+128, out_ch=256)  # 384,256
        self.adfuse4 = Conv1x1(32+32+64+512+32+32+256, out_ch=512)  # 144,512
        self.adfuse5 = Conv1x1(32+32+64+128+512+32+32+64+256, out_ch=512)  # 224,512
        self.adfuse6 = Conv1x1(32+32+64+128+512+32+32+64+256, out_ch=512)  # 224,512
...
class woSFR(nn.Module):
    ...
    def forward(self, x):
        ...
        hx6dup = _upsample_like(self.con_upd6(torch.cat((hx66, hx6), 1)) ,hx5)  # 512 + 512 = 1024 --> 512
        # -------------------- decoder --------------------

        # fusec51,fusec52 = self.fuse5(hx6up, hx5)
        # hx5d = self.stage5d(torch.cat((fusec51, fusec52),1))[0]

        con_up5 = self.con_upd5(hx55 + hx5)  # 512
        hx5d = self.stage5d(torch.cat((con_up5, hx6dup), 1))[0]  # 512 + 512 = 1024 --> 512
        hx5dup = _upsample_like(hx5d, hx4)

        # fusec41,fusec42 = self.fuse4(hx5dup, hx4)
        # hx4d = self.stage4d(torch.cat((fusec41,fusec42),1))[0]
        con_up4 = self.con_upd4(hx44 + hx4)  # 512
        hx4d = self.stage4d(torch.cat((con_up4, hx5dup), 1))[0]  # 512 + 512 = 1024 --> 256
        hx4dup = _upsample_like(hx4d, hx3)

        # fusec31,fusec32 = self.fuse3(hx4dup, hx3)
        # hx3d = self.stage3d(torch.cat((fusec31,fusec32),1))[0]
        con_up3 = self.con_upd3(hx33 + hx3)  # 256
        hx3d = self.stage3d(torch.cat((con_up3, hx4dup), 1))[0]  # 256 + 256 --> 128
        hx3dup = _upsample_like(hx3d, hx2)

        # fusec21, fusec22 = self.fuse2(hx3dup, hx2)
        # hx2d = self.stage2d(torch.cat((fusec21, fusec22), 1))[0]
        con_up2 = self.con_upd2(hx22 + hx2)  # 128
        hx2d = self.stage2d(torch.cat((con_up2, hx3dup), 1))[0]  # 128 + 128 --> 64
        hx2dup = _upsample_like(hx2d, hx1)

        con_up1 = self.con_upd1(hx11 + hx1)   # 64
        hx1d = self.stage1d(torch.cat((con_up1, hx2dup), 1))[0]  # 64 + 64 --> 64
...
class woDB(nn.Module):

    def __init__(self,in_ch=3,out_ch=1):
        super(woDB,self).__init__()
        ...
        # hhl3 中间层设置：
        self.adfuse1 = self.Adaptive_fuse(fnum=1, in_ch=[64], mid_ch=[64], out_ch=64)  # 64,64
        self.adfuse2 = self.Adaptive_fuse(fnum=2, in_ch=[32, 128], mid_ch=[16, 64], out_ch=128)  # 160,128
        self.adfuse3 = self.Adaptive_fuse(fnum=3, in_ch=[32, 32, 256], mid_ch=[16, 16, 64], out_ch=256)  # 320,256
        self.adfuse4 = self.Adaptive_fuse(fnum=4, in_ch=[32, 32, 64, 512], mid_ch=[16, 16, 32, 64],
                                          out_ch=512)  # 640,512
        self.adfuse5 = self.Adaptive_fuse(fnum=5, in_ch=[32, 32, 64, 128, 512], mid_ch=[16, 16, 32, 32, 64],
                                          out_ch=512)  # 768,512
        self.adfuse6 = self.Adaptive_fuse(fnum=5, in_ch=[32, 32, 64, 128, 512], mid_ch=[16, 16, 32, 32, 64],
                                          out_ch=512)  # 768,512
    ...
    def forward(self,x):
        ...
        # stage 1
        hx11, hx12, hx13, hx14, hx15, hx16 = self.stage1(hx)
        hx = self.pool12(hx11)

        # stage 2
        hx22, hx23, hx24, hx25, hx26 = self.stage2(hx)
        hx = self.pool23(hx22)

        # stage 3
        hx33, hx34, hx35, hx36 = self.stage3(hx)
        hx = self.pool34(hx33)

        # stage 4
        hx44, hx45, hx46 = self.stage4(hx)
        hx = self.pool45(hx44)

        # stage 5
        hx55, _ = self.stage5(hx)
        hx = self.pool56(hx55)

        # stage 6
        hx66, _ = self.stage6(hx)
        ...
        hx6dup = _upsample_like(self.con_upd6(torch.cat([hx6,hx66],1)), hx5)
...
class U2NET_conv(nn.Module):
    # feature multilevel
    # concat(ADF,Connection) --> Decoder
    def __init__(self, in_ch=3, out_ch=1):
        super(U2NET_conv, self).__init__()
        ...
        # hhl3 中间层设置：
        self.adfuse1 = Conv1x1(64, 64)
        self.adfuse2 = Conv1x1(32+128, 128)
        self.adfuse3 = Conv1x1(32+32+256, 256)
        self.adfuse4 = Conv1x1(32+32+64+512, 512)
        self.adfuse5 = Conv1x1(32+32+64+128+512, 512)
        self.adfuse6 = Conv1x1(32+32+64+128+512, 512)
    ...
    def forward(self, x):
        ...
        #fusec51,fusec52 = self.fuse5(hx6up, hx5)
        #hx5d = self.stage5d(torch.cat((fusec51, fusec52),1))[0]
        hx5d = self.stage5d(torch.cat((hx6up,self.con_upd5(hx55+hx5)),1))[0]
        hx5dup = _upsample_like(hx5d,hx4)

        #fusec41,fusec42 = self.fuse4(hx5dup, hx4)
        #hx4d = self.stage4d(torch.cat((fusec41,fusec42),1))[0]
        hx4d = self.stage4d(torch.cat((hx5dup,self.con_upd4(hx4+hx44)),1))[0]
        hx4dup = _upsample_like(hx4d,hx3)


        #fusec31,fusec32 = self.fuse3(hx4dup, hx3)
        #hx3d = self.stage3d(torch.cat((fusec31,fusec32),1))[0]
        hx3d = self.stage3d(torch.cat((hx4dup,self.con_upd3(hx3+hx33)),1))[0]
        hx3dup = _upsample_like(hx3d,hx2)

        #fusec21, fusec22 = self.fuse2(hx3dup, hx2)
        #hx2d = self.stage2d(torch.cat((fusec21, fusec22), 1))[0]
        hx2d = self.stage2d(torch.cat((hx3dup,self.con_upd2(hx2+hx22)),1))[0]
        hx2dup = _upsample_like(hx2d,hx1)


        hx1d = self.stage1d(torch.cat((hx2dup,self.con_upd1(hx1+hx11)),1))[0]
...
class halfbranch(nn.Module):
    # 1/2image
    def __init__(self, in_ch=3, out_ch=1):
        super(halfbranch, self).__init__()
         # level1: raw image
        self.stage1 = RSU7(in_ch, 32, 64)
        self.pool12 = nn.MaxPool2d(2, stride=2, ceil_mode=True)
