...
def calculateF1Measure(output, target, thre):
    predict = (F.sigmoid(output).detach().numpy() > thre).astype('int64')  # P
...
class ROCMetric():
    ...
    def get(self):
        ...
        F1_score =  2*recall*precision/np.maximum(0.001,recall+precision)
...
class PD_FA_bins():
    def __init__(self, nclass=1, bins=10):
        super(PD_FA_bins, self).__init__()
        self.nclass = nclass
        self.bins = bins
...
class PD_FA():
    ...
    def update(self, preds, labels):
        ...
        predits  = np.array((preds).detach().numpy()>0).astype('int64')
        labelss = np.array((labels).detach().numpy()).astype('int64')
    ...
    def get(self):
        ...
        Final_PD =  float(self.PD) /float(self.target)
        return Final_PD, Final_FA
...
class rec_prc_F1():
    def __init__(self, thresh = 0.5):
        super(rec_prc_F1, self).__init__()
        self.thresh = thresh
        self.reset()


    def update(self,pred,label):
        recall, prec, F1 = calculateF1Measure(pred,label,self.thresh)
        self.imgnum += 1
        self.recall_sum += recall
        self.precision_sum += prec
        self.F1_sum += F1

    def get(self):
        rec_avg = self.recall_sum / self.imgnum
        prec_avg = self.precision_sum / self.imgnum
        F1_avg = self.F1_sum / self.imgnum
        return rec_avg, prec_avg, F1_avg
